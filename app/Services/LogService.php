...
class CurlService{
  ...
  public function __construct() {
        $this->host     = 'http://project.northtrend.com';
        $this->port     = '80';
...
try {
  ...
  //                       where log.sms = 0 order by log.date_created limit 4"); 


  $stmt = $conn->query("Select user.guardian_contact, user.fullname, log.date_created, log._userId, log.status
  from user_tbl user inner join user_log log
  on user.userId = log._userId
  where log.sms = 0 order by log.date_created limit 10"); 
  ...
  if($result){
      foreach ($result as $value) {
        ...
        // updateSMSlog($conn, $value['_userId'], $value['date_created'], 1 );
        // continue;
