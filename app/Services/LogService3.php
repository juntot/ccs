...
try {
  $stmt = $conn->query("Select user.guardian_contact, user.fullname, log.date_created, log._userId, log.status
                        ...
                        where log.sms = 0 order by log.date_created limit 5, 8 "); 
  ...
  if($result){
      foreach ($result as $value) {
        ...
        $smsResult = sendMsg($value['guardian_contact'], $msg, 7);
        $sms = 1;
        $smsResult = 'success';
        
        if($smsResult == 'failed'){
          sleep(1);
          // $smsResult = sendMsg($value['guardian_contact'], $value['date_created'], $msg, 7);
          $count = 1;
          ...
          // if($smsResult == 'failed')
          // $result = sendMsg($value['guardian_contact'], $value['date_created'], $msg, 7);
